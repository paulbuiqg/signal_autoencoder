...
def batch(conv_rec_params: Tuple[int, Tuple[int, int, int, int, int]]) \
    ...
    x3 = torch.rand((3001, n_channel))
    data = [(x1, 1000), (x2, 2000), (x3, 3001)]
...
def test_conv_rec_encoder(
    ...
    h, _ = s
    assert x.size(0) == x_in.size(0)  # Batch
    assert x.size(1) == x_in.size(1) // 2**3  # Length (pooling)
    assert x.size(2) == h.size(2)  # Features
...
def test_conv_rec_decoder(
    ...
    assert x_in.size(0) == x_out.size(0)  # Batch
    # Length (pooling & upsampling)
    assert x_in.size(1) // 2**3 * 2**3 == x_out.size(1)
    assert x_in.size(2) == x_out.size(2)  # Features
...
def test_conv_rec_autoencoder(
    ...
    assert x_in.size(0) == x_out.size(0)  # Batch
    assert x_in.size(1) // 2**3 * 2**3 == x_out.size(1)  # Length (upsample)
