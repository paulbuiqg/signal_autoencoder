...
# Training
print('Training...')
optimizer = torch.optim.Adam(conv_rec_net.parameters(), lr=.0001)
conv_rec_net.train_one_epoch(device, dataloader, loss_fn, optimizer)
torch.save(conv_rec_net.checkpoint, 'scripts/model.pt')
plt.plot(conv_rec_net.checkpoint['loss_history'])
